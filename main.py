...
# IMPORTANT: FastAPI cannot safely JSON-serialize raw SQLAlchemy ORM objects
# on its own - returning them directly (e.g. `return db.query(Model).all()`)
# throws a "Circular reference detected" error at request time, because the
# ORM instance's __dict__ contains a `_sa_instance_state` object that refers
# back to the instance itself. Every list/detail endpoint below converts rows
# to plain dicts before returning them.
...
TRIAGE_RULES = [
    ...
    },
    {
        ...
        "species": ["Goat", "Sheep"],
        # FIX: originally listed "oral_sores", an id that does not exist in
        # AVAILABLE_SYMPTOMS on the frontend, so this rule could never match.
        # Corrected to "mouth_vesicles", the id the client actually sends.
...
# Safety net: FastAPI only has built-in handlers for HTTPException and
# validation errors. Any other unhandled exception (like the tz-naive vs
# tz-aware datetime bug fixed below) used to propagate past this whole app,
# which meant CORSMiddleware never got a chance to attach its headers to the
# resulting 500 response. The browser can't read a cross-origin response
# without those headers, so it reports the whole thing as a generic
# "Failed to fetch" / network error instead of showing what actually broke.
# Catching it here keeps the response inside CORSMiddleware's wrapper and
# surfaces the real error message to the frontend.
...
class ReportCreate(BaseModel):
    # Enforces exactly 12 numeric digits
...
def submit_report(payload: ReportCreate, db=Depends(get_db), user: UserPayload = Depends(get_current_user)):
    ...
    db_report = ReportModel(
        ...
        reported_by=user.user_id, # Bound securely to the logged-in user
...
def batch_sync_reports(payload: BatchSync, db=Depends(get_db), user: UserPayload = Depends(get_current_user)):
    created_ids = []
    skipped_tags = []
    # FIX: the session uses autoflush=False, so a pending db.add() below is
    # NOT visible to db.query() until commit/flush. Without tracking tags
    # seen earlier in *this* batch, two offline reports queued for the same
    # tag_id could both slip past the "one active report per tag" check and
    # both get inserted. This set closes that gap.
    seen_in_batch = set()
    ...
    for rep in payload.reports:
        ...
        db_report = ReportModel(
            ...
            reported_by=user.user_id, # Bound to current user
...
def get_all_reports(db=Depends(get_db), user: UserPayload = Depends(require_vet_or_admin)):
    # Sorted newest-first by default so "All Reports" has a consistent,
    # predictable order instead of relying on incidental DB insertion order.
    # The frontend additionally supports re-sorting/searching client-side.
    reports = (
...
# Any authenticated user - powers the Farmer "My Reports" tab
...
def detect_disease_clusters(
    ...
    # Defensive validation: an empty/cleared radius or min-cases field on the
    # frontend becomes 0, which reaches sklearn as eps=0 / min_samples=0 and
    # DBSCAN raises a raw ValueError for that - previously an unhandled 500
    # that also showed up in the browser as "Failed to fetch".
    if min_cases < 1:
    ...
    # BUG FIX (root cause of "Failed to fetch" on this endpoint):
    # r.server_received_at comes back from SQLite as a naive datetime (see
    # _ensure_utc above for why), while time_threshold is tz-aware. Comparing
    # them directly raised "TypeError: can't compare offset-naive and
    # offset-aware datetimes" on every single request to this endpoint. That
    # exception used to propagate past CORSMiddleware entirely, so the
    # response had no CORS headers and the browser reported it as a generic
...
def create_sample_referral(payload: SampleCreate, db=Depends(get_db), user: UserPayload = Depends(require_vet_or_admin)):
    ...
    sample = SampleModel(
        ...
        collected_by=user.user_id, # Bound to current vet
...
# Restricted to Vet/Admin - powers the "Lab Samples" tab
...
def delete_report(report_id: str, db=Depends(get_db), user: UserPayload = Depends(get_current_user)):
    ...
    if user.role != "ADMIN":
        ...
        # 3. Check 14-day rule (SQLite stores timezone-naive UTC times by default)
...
def resolve_report(report_id: str, db=Depends(get_db), user: UserPayload = Depends(require_vet_or_admin)):
    ...
    report.cluster_id = None  # a resolved case shouldn't keep displaying a stale outbreak-cluster tag
    db.commit()
    return {
        "status": "success", 
